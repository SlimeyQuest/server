...
func (s *Service) PhoneRegister(ctx context.Context, req *apitypes.PhoneRegisterReq) (*apitypes.AuthResponse, *AuthResult) {
	if req == nil {
		return authError(apitypes.ErrorCodeInvalidRequest, "missing phone register payload"), nil
	}
...
func (s *Service) PhoneLogin(ctx context.Context, req *apitypes.PhoneLoginReq) (*apitypes.AuthResponse, *AuthResult) {
	if req == nil {
		return authError(apitypes.ErrorCodeInvalidRequest, "missing phone login payload"), nil
	}
	return s.phoneAuth(ctx, req.Phone, req.VerifyCode, req.ClientVersion, false)
}

func (s *Service) phoneAuth(ctx context.Context, phone, verifyCode, clientVersion string, register bool) (*apitypes.AuthResponse, *AuthResult) {
	if phone == "" {
		return authError(apitypes.ErrorCodeInvalidRequest, "phone is required"), nil
	}
	if !validTestVerifyCode(verifyCode) {
		return authError(apitypes.ErrorCodeInvalidRequest, "invalid verify code"), nil
	}
