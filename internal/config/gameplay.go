...
// GameplayConfig holds gameplay balance tables loaded at startup.
...
// LoadGameplay parses embedded gameplay config files.
func LoadGameplay() (*GameplayConfig, error) {
...
func (c *GameplayConfig) GoldPerSec(highestStageCleared int32) float64 {
...
func (c *GameplayConfig) Stage(flat int32) (StageRow, bool) {
	row, ok := c.Stages[flat]
	return row, ok
}

// RecommendedPower returns gate power for a flat stage including boss multiplier.
func (c *GameplayConfig) RecommendedPower(flat int32) int64 {
	row, ok := c.Stages[flat]
	if !ok {
		return 0
	}
...
func (c *GameplayConfig) StageIdleScale(highestStageCleared int32) float64 {
...
func (c *GameplayConfig) ClearThreshold() float64 {
...
func (c *GameplayConfig) PickIdleDrop(seed int64) DropRow {
	total := 0
