...
func (c *Conn) sessionBinding() login.SessionBinding {
	return login.SessionBinding{ID: c.id, Handle: c}
}

func (c *Conn) applyAuth(auth *login.AuthResult) {
	if auth == nil {
		return
	}
	if auth.ReplacedBinding != nil {
		if oldConn, ok := auth.ReplacedBinding.Handle.(*Conn); ok && oldConn != c {
			oldConn.Close()
		}
	}
	c.SetAuthenticated(auth.PlayerID, auth.SessionToken)
}
...
func (c *Conn) handleGuestLogin(req *loginv1.GuestLoginReq) bool {
	...
	res, auth := c.loginSvc.GuestLogin(c.ctx, c.sessionBinding(), req)
...
func (c *Conn) handlePhoneRegister(req *loginv1.PhoneRegisterReq) bool {
	res, auth := c.loginSvc.PhoneRegister(c.ctx, c.sessionBinding(), req)
...
func (c *Conn) handlePhoneLogin(req *loginv1.PhoneLoginReq) bool {
	res, auth := c.loginSvc.PhoneLogin(c.ctx, c.sessionBinding(), req)
	if res.GetError().GetCode() != commonv1.ErrorCode_ERROR_CODE_OK {
