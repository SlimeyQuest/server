...
func (r *Repository) LoadProgress(ctx context.Context, playerID int64) (*playersvc.ProgressState, error) {
	p, err := r.GetByID(ctx, playerID)
	if err != nil {
		return nil, err
	}
	return playersvc.FromEntity(p), nil
}
...
func (r *Repository) CreatePlayerForPlatform(ctx context.Context, platform, externalID, nickname string) (*ent.Player, error) {
	now := time.Now()
	equip := playersvc.StarterEquipment(r.cfg)
	p, err := r.client.Player.
		Create().
...
func (r *Repository) SaveRole(ctx context.Context, state *playersvc.ProgressState) error {
	if state == nil {
...
func (r *Repository) SaveProgress(ctx context.Context, state *playersvc.ProgressState) error {
	if state == nil {
	...
	upd := r.client.Player.UpdateOneID(int(state.PlayerID)).
		...
		SetEquipmentJSON(playersvc.EncodeEquipment(state.Equipment)).
